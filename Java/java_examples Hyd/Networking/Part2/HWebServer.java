...
import java.net.*;
import java.util.*;
class HWebServer implements HttpConstants {
...
class Worker extends HWebServer implements HttpConstants, Runnable {
    ...
    public synchronized void run() {
        while(true) {
             ...
             */
            s = null;
            Vector pool = HWebServer.threads;
            synchronized (pool) {
                if (pool.size() >= HWebServer.workers) {
    ...
    void handleClient() throws IOException {
         ...
         */
        s.setSoTimeout(HWebServer.timeout);
...
outerloop:
            ...
            File targ = new File(HWebServer.root, fname);
            if (targ.isDirectory()) {
